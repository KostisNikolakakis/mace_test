...
class LRScheduler:
    def __init__(self, optimizer, args) -> None:
        ...
        ######################################################################################################
        ######################### Modified LRscheduler to include Cosine Annealing and StepLR
        elif args.scheduler == "CosineAnnealingLR":
            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=optimizer, 
                T_max = args.max_num_epochs, #len(train_loader) * args.epochs, ### removed the len(train_loader) and decay in every epoch 
                eta_min=args.min_lr # make sure you get min_lr from input
            )
            #scheduler.last_epoch = (start_epoch - 1) * len(train_loader) # required possibly for checkpoint retrieval
        elif args.scheduler == "StepLR":
            self.lr_scheduler = torch.optim.lr_scheduler.StepLR(
                optimizer=optimizer, 
                step_size= args.step_size, #len(train_loader) * args.epochs, ### removed the len(train_loader) and decay in every epoch
                gamma=args.gamma
            )
            #scheduler.last_epoch = (start_epoch - 1) * len(train_loader) # required possibly for checkpoint retrieval
        #else: 
        #    sel.lr_scheduler = torch.optim.lr_scheduler.__dict__[args.scheduler](optimizer)
        
        ################# Check again the else above #########################################################
        ######################################################################################################
        else:
    ...
    def step(self, metrics=None, epoch=None):  # pylint: disable=E1123
        ...
        ################## added the two lines below , check for missing arguments in the step function    
